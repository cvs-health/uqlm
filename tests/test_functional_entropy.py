...
@pytest.fixture
def fe(mock_llm):
    return FunctionalEntropy(
        equivalence_llm=mock_llm,
        language="python"
    )
...
def test_semantic_entropy_process():
    ...
    discrete, tokenprob, num_sets = fe._semantic_entropy_process(
        single_prompt_cluster_indices=cluster_indices,
...
def test_compute_response_probabilities():
    ...
    tokenprob, resp_probs = fe._compute_response_probabilities(
        logprobs_results=logprobs,
        num_responses=2
    )
