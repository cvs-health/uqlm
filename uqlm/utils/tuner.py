...
class Tuner:
    def __init__(self) -> None:
        ...
        self.objective_to_func = {
            ...
            "auprc": self._auprc,  # area under precision-recall curve
        }
    ...
    def _brier_score(self, y_true, y_pred):
        """Helper function to compute Brier score"""
        y_true = np.array(y_true)
        y_pred = np.array(y_pred)
        return np.mean((y_true - y_pred) ** 2)

    def _auprc(self, y_true, y_prob):
        """Helper function to compute area under precision-recall curve"""
        desc_sort = np.argsort(-np.array(y_prob))
        y_true = np.array(y_true)[desc_sort]
        y_prob = np.array(y_prob)[desc_sort]
