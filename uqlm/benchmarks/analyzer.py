...
class BenchmarkAnalyzer:
    ...
    def compare_scorers(self, run_ids: Optional[List[str]] = None, benchmark_name: Optional[str] = None, benchmark_category: Optional[str] = None, llm_names: Optional[List[str]] = None, date_range: Optional[Tuple[datetime, datetime]] = None) -> pd.DataFrame:
        """
        ...
        """
        # Get runs to analyze
        runs = self._get_runs(run_ids, benchmark_name, benchmark_category, llm_names, None, date_range)
                ...
                comparison_data.append(
                    {
                        ...
                        "benchmark_category": run.metadata.config.benchmark_category,
    ...
    def compare_llms(self, run_ids: Optional[List[str]] = None, benchmark_name: Optional[str] = None, benchmark_category: Optional[str] = None, scorer_names: Optional[List[str]] = None, date_range: Optional[Tuple[datetime, datetime]] = None) -> pd.DataFrame:
        """
        ...
        """
        # Get runs to analyze
        runs = self._get_runs(run_ids, benchmark_name, benchmark_category, None, scorer_names, date_range)
    ...
    def aggregate_metrics(self, run_id: str, groupby: str = "llm") -> Dict[str, Dict[str, float]]:
        """
    ...
    def plot_results(self, comparison_df: pd.DataFrame, plot_type: str = "bar", save_path: Optional[str] = None, **plot_kwargs):
        """
    ...
    def export_report(self, run_ids: List[str], output_path: str, format: str = "html"):
        """
    ...
    def list_runs(self, benchmark_name: Optional[str] = None, benchmark_category: Optional[str] = None, limit: int = 20) -> pd.DataFrame:
        """
        ...
        """
        runs_metadata = self.db.query_runs(benchmark_name=benchmark_name, benchmark_category=benchmark_category, status="completed", limit=limit)
            ...
            data.append(
                {
                    ...
                    "benchmark_category": metadata.config.benchmark_category,
    ...
    def _get_runs(self, run_ids: Optional[List[str]], benchmark_name: Optional[str], benchmark_category: Optional[str], llm_names: Optional[List[str]], scorer_names: Optional[List[str]], date_range: Optional[Tuple[datetime, datetime]]) -> List[BenchmarkRun]:
        """Helper to get runs based on IDs or filters."""
        if run_ids:
            return [self.db.load_run(run_id) for run_id in run_ids]
        else:
            metadata_list = self.db.query_runs(benchmark_name=benchmark_name, benchmark_category=benchmark_category, llm_names=llm_names, scorer_names=scorer_names, date_range=date_range, status="completed")
    ...
    def _aggregate_by_scorer(self, run: BenchmarkRun) -> Dict[str, Dict[str, float]]:
        ...
        for scorer_name, scores in scorer_metrics.items():
            series = pd.Series(scores)
            aggregated[scorer_name] = {"mean": series.mean(), "std": series.std(), "min": series.min(), "max": series.max(), "count": len(scores)}

        return aggregated

    def _plot_llm_comparison(self, df: pd.DataFrame, plot_type: str, save_path: Optional[str], **kwargs):
        """Plot LLM comparison."""
    ...
    def _plot_scorer_comparison(self, df: pd.DataFrame, plot_type: str, save_path: Optional[str], **kwargs):
