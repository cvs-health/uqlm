...
def validate_benchmark_scorers(benchmark: Any, scorers: List[str], benchmark_category: str) -> None:
    ...
    benchmark_category : str
        The benchmark category (from benchmark.get_supported_category())
...
def validate_benchmark_implementation(benchmark: Any) -> None:
    ...
    """
    required_methods = [("get_name", "Get the benchmark name"), ("get_supported_category", "Declare the supported benchmark category"), ("get_dataset_name", "Get the HuggingFace dataset name"), ("get_prompts", "Retrieve prompts for evaluation")]
    ...
    if missing_methods:
        raise BenchmarkValidationError(f"\n{'=' * 70}\nINVALID BENCHMARK IMPLEMENTATION\n{'=' * 70}\n\nThe benchmark {benchmark.__class__.__name__} is missing required methods:\n\n{chr(10).join(missing_methods)}\n\nAll benchmarks must inherit from BaseBenchmark and implement these methods.\n{'=' * 70}")

    # Validate that methods are callable
    for method_name, _ in required_methods:
        method = getattr(benchmark, method_name, None)
        if not callable(method):
            raise BenchmarkValidationError(f"Benchmark {benchmark.__class__.__name__}: {method_name} exists but is not callable.")
